...
def list_arbitros(request):
    ...
    if search != "" and export == "":
        ...
        if rol_user == 1 or rol_user == 2:
            group_by='group by ua.usuario_id,au.first_name,au.last_name,au.email,ua.id,au.username,arb.genero,arb.cedula_pasaporte,arb.titulo,arb.linea_investigacion,arb.telefono_habitacion_celular'
            ...
            search= search+'%'
            where="WHERE asa.sistema_asovac_id=%s and ris.sistema_asovac_id={} and (arb.nombres like %s or arb.apellidos like %s or arb.genero like %s or arb.correo_electronico like %s or arb.titulo like %s or arb.cedula_pasaporte like %s or arb.linea_investigacion like %s) ".format(arbitraje_id)
            ...
            if rol_user == 3:
                ...
                where="WHERE asa.sistema_asovac_id=%s and ris.sistema_asovac_id={} and a.id in ({}) and  (arb.nombres like %s or arb.apellidos like %s or arb.genero like %s or arb.correo_electronico like %s or arb.titulo like %s or arb.cedula_pasaporte like %s or arb.linea_investigacion like %s) ".format(arbitraje_id,area)
        ...
        if rol_user == 1 or rol_user ==2 :
            ...
            if rol_user == 3:
                data= User.objects.raw(query,[arbitraje_id,search,search,search,search,search,search,search])
                data_count= User.objects.raw(query_count,[arbitraje_id,search,search,search,search,search,search,search])
        ...
        if export !=  "":
            ...
            if rol_user == 1 or rol_user == 2:
                where="WHERE asa.sistema_asovac_id={} and ris.sistema_asovac_id={} ".format(arbitraje_id,arbitraje_id)
...
def list_subareas_arbitros(request,id):
    ...
    return JsonResponse(response)
#---------------------------------------------------------------------------------#
#          Carga el contenido de la tabla de subáreas para usuarios               #
#---------------------------------------------------------------------------------#
@login_required
@user_is_arbitraje
def list_subareas_usuarios(request,id):
    ...
    # consulta mas completa
    query= "select usu_sub_a.id, sub_a.nombre, sub_a.codigo, sub_a.descripcion from main_app_usuario_asovac as usu_aso inner join main_app_usuario_rol_in_sistema as ur_sis on ur_sis.usuario_asovac_id = usu_aso.id inner join main_app_usuario_asovac_sub_area as usu_sub_a on usu_sub_a.usuario_asovac_id = usu_aso.id inner join main_app_sub_area as sub_a on sub_a.id = usu_sub_a.sub_area_id inner join main_app_area as ar on ar.id = sub_a.area_id"
    where=' WHERE ur_sis.sistema_asovac_id =%s AND usu_aso.id=%s '
    # where=' WHERE sis_aso.sistema_asovac_id =1 and arb.id=8 '
    query= query+where
    query_count=query
...
def list_arbitrajes(request):
    ...
    for item in data:
        ...
        autor_principal=item.lista_autores
        area = item.nombre
        ...
        response['query'].append({'id':item.id,'archivo_trabajo': item.archivo_trabajo ,'estatus': estatus ,'nombre':area,'titulo_espanol':titulo ,'forma_presentacion':presentacion, 'observaciones':observaciones,'revision':revision, 'autor_principal':autor_principal,'subarea':subarea })
...
def list_trabajos_aceptados(request):
    ...
    if search != "" and export == "":
        ...
        if rol_user == 1 or rol_user == 2:
            ...
            query= "SELECT DISTINCT(trab.id),trab.estatus,trab.requiere_arbitraje,trab.titulo_espanol,trab.forma_presentacion,main_a.nombre,trab.observaciones,STRING_AGG (distinct(concat(aut.nombres,' ',aut.apellidos)), ', ') lista_autores,main_sarea.nombre as subarea FROM trabajos_trabajo AS trab INNER JOIN autores_autores_trabajos AS aut_trab ON aut_trab.trabajo_id = trab.id INNER JOIN autores_autor AS aut ON aut.id = aut_trab.autor_id INNER JOIN main_app_sistema_asovac AS sis_aso ON sis_aso.id = aut_trab.sistema_asovac_id INNER JOIN trabajos_trabajo_subareas AS trab_suba ON trab_suba.trabajo_id = trab.id INNER JOIN main_app_sub_area AS main_sarea on main_sarea.id = trab_suba.sub_area_id INNER JOIN main_app_area AS main_a ON main_a.id= main_sarea.area_id"
            query_count=query
            search= search+'%'
...
def generate_report(request,tipo):
    ...
    if tipo == '1':
        ...
        if rol_user == 1 or rol_user == 2:
                where="WHERE asa.sistema_asovac_id={} ".format(arbitraje_id)
                query= "SELECT DISTINCT ua.usuario_id, au.first_name,au.last_name, au.email,ua.id, au.username, STRING_AGG (distinct(a.nombre), ', ') nombre, arb.genero, arb.cedula_pasaporte,arb.titulo, arb.linea_investigacion, arb.telefono_habitacion_celular FROM main_app_usuario_asovac AS ua INNER JOIN auth_user AS au ON ua.usuario_id = au.id INNER JOIN main_app_usuario_asovac_sub_area AS uasa ON uasa.usuario_asovac_id= ua.id INNER JOIN main_app_sub_area AS sa ON sa.id= uasa.sub_area_id INNER JOIN main_app_area AS a ON a.id = sa.area_id INNER JOIN main_app_usuario_rol_in_sistema AS ris ON ris.usuario_asovac_id = ua.id INNER JOIN arbitrajes_arbitro AS arb ON arb.usuario_id = ua.id INNER JOIN "+'"arbitrajes_arbitro_Sistema_asovac"'+" AS asa ON asa.arbitro_id = arb.id {} group by ua.usuario_id,au.first_name,au.last_name,au.email,ua.id,au.username,arb.genero,arb.cedula_pasaporte,arb.titulo,arb.linea_investigacion,arb.telefono_habitacion_celular ".format(where)
        
        else:
            if rol_user == 3:
                where=' WHERE a.id in ({}) AND asa.sistema_asovac_id={} '.format(area,arbitraje_id)
        ...
        response['Content-Disposition'] = 'attachment; filename=convencion_asovac_{}_lista_de_arbitros.xls'.format(date)
...
def descargar_trabajo(request, trabajo_id):

    data = dict()

    media_root = settings.MEDIA_ROOT
    trabajo = Trabajo.objects.get(id = trabajo_id)
    data['url'] = str(trabajo.archivo_trabajo)
    return JsonResponse(data)